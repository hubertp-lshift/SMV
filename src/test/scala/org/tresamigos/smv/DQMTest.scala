...
class DQMTest extends SmvTestUtil {
  ...
  test("test DQMRule") {
    ...
    val dqmRule1     = DQMRule((new Column("a")) + (new Column("b")) > 0.3, Some("rule1"))
  ...
  test("test FailPercent") {
    ...
    val dqm = new DQMValidator(
      SmvDQM()
        .add(DQMRule($"b" < 0.4, Some("b_lt_03"), FailPercent(0.5)))
        .add(DQMFix($"a" < 1, lit(1) as "a", Some("a_lt_1_fix"), FailPercent(0.3))))
  ...
  test("test Total Policies") {
    ...
    val dqm = new DQMValidator(
      SmvDQM()
        .add(DQMRule($"b" < 0.4, Some("b_lt_03")))
        .add(DQMFix($"a" < 1, lit(1) as "a", Some("a_lt_1_fix")))
  ...
  test("test user defined policy") {
    ...
    object file extends SmvCsvStringData("a:Integer;b:Double", "1,0.3;0,0.2;3,0.5") {
      ...
      override def dqm() =
        SmvDQM()
          .add(DQMRule($"b" < 0.4, Some("rule1")))
          .add(DQMFix($"a" < 1, lit(1) as "a", Some("fix2")))
